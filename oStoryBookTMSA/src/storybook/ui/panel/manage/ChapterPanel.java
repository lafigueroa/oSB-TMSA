...
public class ChapterPanel extends AbstractPanel implements
		IRefreshable {
	...
	public void modelPropertyChange(PropertyChangeEvent evt) {
		...
		if (BookController.ChapterProps.UPDATE.check(propName)) {
			...
			if (newChapter.getId() != chapter.getId()) {
				return;
			}
			chapter = newChapter;
			refresh();
		...
		if (BookController.SceneProps.UPDATE.check(propName)) {
			...
			if (newSceneChapter.getId() != chapter.getId()
					&& oldSceneChapter.getId() != chapter.getId()) {
				return;
			}
			refresh();
			return;
	...
	public void initUi() {
		...
		if (chapter == null) {
			...
			for (Scene scene : scenes) {
				...
				SwingUtil.setForcedSize(dtScene, new Dimension(prefWidth - 10,
						140));
				add(dtScene, "growy");
			}
			// to make a scene unassigned
			DTScenePanel makeUnassigned = new DTScenePanel(mainFrame,
			...
			DTScenePanel begin = new DTScenePanel(mainFrame,
					DTScenePanel.TYPE_BEGIN);
			...
			for (Scene scene : scenes) {
				...
				// move next
				DTScenePanel next = new DTScenePanel(mainFrame,
						DTScenePanel.TYPE_NEXT);
